...
def index():
    ...
    rev_table = playstore.nlargest(10, 'Reviews')[['Category', 'App', 'Reviews', 'Rating']].reset_index(drop=True)
    ...
    # Ubah kolom Last Updated menjadi datetime
    ...
    # Hitung jumlah installasi per bulan
    df2['MonthYear'] = pd.to_datetime(df2[['Year', 'Month']].assign(DAY=1))
    monthly_installs = df2.groupby('MonthYear')['Installs'].sum().reset_index()

    ## Buatlah sebuah plot yang menampilkan insight di dalam data 
    plt.figure(figsize=(10, 6))
    plt.plot(monthly_installs['MonthYear'], monthly_installs['Installs'])

    # Set label dan judul
    plt.xlabel('Month-Year')
    plt.ylabel('Number of Installs')
    plt.title('Trend of Application Installation Count per Month')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('line_size.png',bbox_inches="tight")
